...
func DeleteRoomHandler(w http.ResponseWriter, r *http.Request) {
	...
	newResource, _ := messaging.ReassembleResource(eventResource, roomPayload)
	_ = messaging.GetCentrifugoService().Publish(ctx, newResource)

	members, _ := dao.GetUsersByRoom(ctx, roomPayload.ID)
	for _, m := range members {
		_ = messaging.GetCentrifugoService().Publish(ctx, &messaging.EventResource{
			Channel:   fmt.Sprintf("user:%d:rooms", m.ID),
			Method:    "DELETE",
