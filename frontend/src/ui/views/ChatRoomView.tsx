      ...
      <div className="flex-1 flex flex-col overflow-y-auto space-y-3 px-[2%] py-4">
        {props.messageList.map((m) => {
          ...
          return (
            <div
              key={m.id}
              className={`group relative w-4/5 flex rounded-lg ${bubbleBg} py-[0.45rem] pb-[0.6rem] ${
                isMine ? "" : "flex-row-reverse"
              }`}
            >
              <div
                className={`flex flex-col items-center px-4 border-gray-600 ${
                  isMine ? "border-r" : "border-l"
                }`}
              >
                <img
                  src={
              ...
              <div className="flex-1 min-w-0 px-2 leading-relaxed whitespace-pre-wrap">
                ...
                <div className="break-words">{m.contents}</div>
              </div>
